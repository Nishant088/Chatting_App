...
exports.getUsersList = async (req, res) => {
    try {
        //we want to get all the users except the current user
        const userList = await getUsersList(req.params.id);
        //userlist will be [{name: 'user1', sessionId: '123'},{name: 'user2', sessionId: '223'}]
        let userListObj = {};
        //userlistObj will be
        //{'123': {name: 'user1', sessionId: '123'}, '223': {name: 'user2', sessionId: '223'}}
        //it will become easy to show data using this 
        if (userList.length) {
